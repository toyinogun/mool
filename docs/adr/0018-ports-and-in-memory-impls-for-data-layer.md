...
- The in-memory impls are exported from `app/src/auth/authStore.ts` and `app/src/recording.ts`. Production wiring picks the Postgres impl when a DB handle is available; the in-memory impls serve as a defensive fallback only on the `skipDb: true` test path (which throws in production via a guard in `server.ts`).
